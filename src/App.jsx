...
// ─── FB ACCOUNTS TAB ──────────────────────────────────────────────────────
const EMPTY_FB = { account_id:"", login:"", status:"живий", buyer_id:"", bm_name:"", ad_cabinet:"", balance:0, spend_limit:0, comment:"" };

const FbForm = ({ initial={}, buyers, onSave, onClose, loading }) => {
  const [f, setF] = useState({ ...EMPTY_FB, ...initial });
  const set = k => e => setF(p=>({...p,[k]:e.target.value}));
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <Field label="Account ID"><input style={S.inp} value={f.account_id} onChange={set("account_id")} placeholder="act_123456789" /></Field>
        <Field label="Логін / Email"><input style={S.inp} value={f.login} onChange={set("login")} placeholder="user@gmail.com" /></Field>
        <Field label="Статус">
          <select style={{ ...S.inp, cursor:"pointer" }} value={f.status} onChange={set("status")}>
        ...
        <Field label="BM / Кабінет"><input style={S.inp} value={f.bm_name} onChange={set("bm_name")} placeholder="BM назва" /></Field>
        <Field label="Рекламний кабінет"><input style={S.inp} value={f.ad_cabinet} onChange={set("ad_cabinet")} placeholder="Назва кабінету" /></Field>
        <Field label="Баланс ($)"><input style={{ ...S.inp, textAlign:"right" }} type="number" value={f.balance} onChange={set("balance")} /></Field>
        <Field label="Ліміт ($)"><input style={{ ...S.inp, textAlign:"right" }} type="number" value={f.spend_limit} onChange={set("spend_limit")} /></Field>
      </div>
      <Field label="Коментар"><textarea style={{ ...S.inp, height:64, resize:"vertical" }} value={f.comment} onChange={set("comment")} /></Field>
      <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:8 }}>
        <button onClick={onClose} style={S.btnGhost}>Скасувати</button>
        <button onClick={()=>onSave(f)} disabled={loading} style={{ ...S.btn, opacity:loading?0.7:1 }}>{loading?"…":"Зберегти"}</button>
      </div>
...
const FbAccountsTab = ({ user, isAdmin, canSeeAll }) => {
  ...
  const fetchAll = async () => {
    ...
    if (accounts) setRows(accounts);
    if (profiles) setBuyers(profiles.filter(p=>p.role==="buyer"||p.role==="admin"));
    setLoading(false);
  };
  useEffect(()=>{ fetchAll(); },[]);

  const filtered = rows.filter(a=>{
    const buyerName = buyers.find(b=>b.id===a.buyer_id)?.full_name||"";
    const text = [a.account_id,a.login,a.bm_name,a.ad_cabinet,buyerName].join(" ").toLowerCase();
  ...
  const handleSave = async (f) => {
    setSaving(true);
    const payload = { ...f, user_id:user.id, balance:parseFloat(f.balance)||0, spend_limit:parseFloat(f.spend_limit)||0 };
    if (modal.mode==="add") { delete payload.id; await supabase.from("fb_accounts").insert([payload]); showToast("Акаунт додано ✓"); }
    else { await supabase.from("fb_accounts").update(payload).eq("id",f.id); showToast("Збережено ✓"); }
    setSaving(false); setModal(null); fetchAll();
  };

  const del = async (id) => { if (!confirm("Видалити?")) return; await supabase.from("fb_accounts").delete().eq("id",id); showToast("Видалено"); fetchAll(); };

  const getBuyer = id => buyers.find(b=>b.id===id);

  const stats = {
    total: rows.length,
    alive: rows.filter(a=>a.status==="живий").length,
    warm:  rows.filter(a=>a.status==="на прогріві").length,
    banned:rows.filter(a=>a.status==="забанений").length,
  };
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        {[["Всього акаунтів",stats.total,"#60a5fa"],["Живих",stats.alive,"#4ade80"],["На прогріві",stats.warm,"#fbbf24"],["Забанених",stats.banned,"#f87171"]].map(([l,v,c])=>(
          <div key={l} style={S.card}><div style={{ color:"#64748b",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em" }}>{l}</div><div style={{ color:c,fontSize:26,fontWeight:800,marginTop:4 }}>{v}</div></div>
        ))}
      </div>

      <div style={{ display:"flex", gap:12, marginBottom:16, alignItems:"center", flexWrap:"wrap" }}>
        <input style={{ ...S.inp, maxWidth:260 }} placeholder="🔍 Пошук…" value={filter} onChange={e=>setFilter(e.target.value)} />
        <select style={{ ...S.inp, maxWidth:180, cursor:"pointer" }} value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}>
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#13151c", borderRadius:10, overflow:"hidden" }}>
            ...
            <tbody>
              ...
              {filtered.map(a=>{
                const buyer = getBuyer(a.buyer_id);
                return (
                  <tr key={a.id} onMouseEnter={e=>e.currentTarget.style.background="#1e2330"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                    <Td style={{ fontWeight:600, color:"#e2e8f0", fontFamily:"monospace" }}>{a.account_id||"—"}</Td>
                    <Td style={{ color:"#60a5fa", fontSize:12 }}>{a.login||"—"}</Td>
                    <Td><Badge s={a.status} /></Td>
                    <Td>{buyer?<div><div style={{ fontSize:13 }}>{buyer.full_name}</div></div>:<span style={{ color:"#475569" }}>—</span>}</Td>
                    <Td style={{ color:"#94a3b8" }}>{a.bm_name||"—"}</Td>
                    <Td style={{ color:"#94a3b8" }}>{a.ad_cabinet||"—"}</Td>
      ...
      {modal&&<Modal title={modal.mode==="add"?"Новий FB акаунт":"Редагувати акаунт"} onClose={()=>setModal(null)}><FbForm initial={modal.data} buyers={buyers} onSave={handleSave} onClose={()=>setModal(null)} loading={saving} /></Modal>}
    </div>
  );
};
