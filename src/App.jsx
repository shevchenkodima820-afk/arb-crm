  ...
  <div style={{ position:"fixed", inset:0, background:"#000b", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center" }}>
    <div style={{ background:"#1a1d23", border:"1px solid #2e3240", borderRadius:14, width:"min(640px,96vw)", maxHeight:"90vh", overflowY:"auto", padding:28 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        ...
        <button onClick={onClose} style={{ background:"none", border:"none", color:"#64748b", fontSize:24, cursor:"pointer", lineHeight:1, padding:"0 4px" }}>×</button>
      </div>
      {children}
    </div>
  </div>
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        if (error) throw error;
        setError("✅ Перевірте email для підтвердження!");
      ...
      <div style={{ ...S.card, width:"min(400px,94vw)", padding:36 }}>
        ...
        <div style={{ display:"flex", background:"#0f1117", borderRadius:8, padding:3, marginBottom:20 }}>
          {[["login","Увійти"],["signup","Реєстрація"]].map(([m,l]) => (
            <button key={m} onClick={() => { setMode(m); setError(""); }} style={{ flex:1, padding:"8px", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontSize:13, background:mode===m?"#1e2330":"transparent", color:mode===m?"#e2e8f0":"#64748b", transition:"all 0.15s" }}>{l}</button>
          ))}
        </div>

        {mode === "signup" && <Field label="Ім'я байєра"><input style={S.inp} value={name} onChange={e=>setName(e.target.value)} placeholder="Ваше ім'я" /></Field>}
        ...
        {error && <div style={{ color: error.startsWith("✅") ? "#4ade80" : "#f87171", fontSize:13, marginBottom:12, padding:"8px 12px", background: error.startsWith("✅") ? "#16a34a22" : "#dc262622", borderRadius:8 }}>{error}</div>}

        <button onClick={submit} disabled={loading} style={{ ...S.btn, width:"100%", padding:"11px", fontSize:15, opacity:loading?0.7:1 }}>
  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <Field label="Домен (URL)"><input style={S.inp} value={f.domain} onChange={set("domain")} placeholder="example.com" /></Field>
        <Field label="ПВА назва"><input style={S.inp} value={f.pwa} onChange={set("pwa")} placeholder="MyApp_UA" /></Field>
        <Field label="Оффер"><input style={S.inp} value={f.offer} onChange={set("offer")} placeholder="1win, Pin-up…" /></Field>
        <Field label="ГЕО"><input style={S.inp} value={f.geo} onChange={set("geo")} placeholder="UA, PL, BR…" /></Field>
        <Field label="Статус">
          <select style={sel} value={f.status} onChange={set("status")}>
            {["активний","на паузі","мертвий"].map(s=><option key={s}>{s}</option>)}
          </select>
        </Field>
        <Field label="Байєр"><input style={S.inp} value={f.buyer} onChange={set("buyer")} placeholder="Ім'я байєра" /></Field>
...
const DomainsTab = ({ user }) => {
  ...
  const filtered = rows.filter(d =>
    [d.domain,d.pwa,d.offer,d.geo,d.buyer,d.status].join(" ").toLowerCase().includes(filter.toLowerCase())
  );
  ...
  const handleSave = async (f) => {
    ...
    if (modal.mode === "add") {
      ...
      if (error) { showToast("Помилка збереження", "error"); }
      else { showToast("Домен додано ✓"); }
    } else {
      const { error } = await supabase.from("domains").update(payload).eq("id", f.id);
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign:"center", color:"#475569", padding:40 }}>Завантаження…</div> : (
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#13151c", borderRadius:10, overflow:"hidden" }}>
            <thead>
              <tr>
                <Th>Домен</Th><Th>ПВА</Th><Th>Оффер</Th><Th>ГЕО</Th><Th>Статус</Th><Th>Байєр</Th><Th>Запуск</Th><Th>Коментар</Th><Th></Th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={9} style={{ padding:40, textAlign:"center", color:"#475569" }}>Немає записів — додайте перший домен</td></tr>}
              {filtered.map(d => (
                <tr key={d.id} onMouseEnter={e=>e.currentTarget.style.background="#1e2330"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                  ...
                  <Td>
                    <button onClick={() => setModal({ mode:"edit", data:d })} style={{ background:"none", border:"none", color:"#60a5fa", cursor:"pointer", fontSize:14 }}>✏️</button>
                    <button onClick={() => del(d.id)} style={{ background:"none", border:"none", color:"#f87171", cursor:"pointer", fontSize:14 }}>🗑</button>
                  </Td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && (
        <Modal title={modal.mode==="add"?"Новий домен":"Редагувати домен"} onClose={() => setModal(null)}>
          <DomainForm initial={modal.data} onSave={handleSave} onClose={() => setModal(null)} loading={saving} />
        </Modal>
      )}
    </div>
  );
...
const CreativeForm = ({ initial={}, domains, onSave, onClose, loading, userId }) => {
  ...
  const set = k => e => setF(p => ({ ...p, [k]: e.target.value }));
  const sel = { ...S.inp, cursor:"pointer" };
  const num = { ...S.inp, textAlign:"right" };
  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <Field label="Назва / ID крео"><input style={S.inp} value={f.name} onChange={set("name")} placeholder="creo_001" /></Field>
        <Field label="Байєр"><input style={S.inp} value={f.buyer} onChange={set("buyer")} placeholder="Ім'я байєра" /></Field>
        <Field label="Домен / ПВА">
          <select style={sel} value={f.domain_id} onChange={set("domain_id")}>
            <option value="">— не прив'язаний —</option>
            {domains.map(d => <option key={d.id} value={d.id}>{d.domain} ({d.pwa})</option>)}
          </select>
        </Field>
        <Field label="Статус">
          <select style={sel} value={f.status} onChange={set("status")}>
        ...
        <Field label="Дата додавання"><input style={S.inp} type="date" value={f.added_date} onChange={set("added_date")} /></Field>
      </div>

      <Field label="Прев'ю">
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <input style={{ ...S.inp, flex:1 }} value={f.preview_url} onChange={set("preview_url")} placeholder="URL або завантажте файл →" />
          <button onClick={() => fileRef.current.click()} disabled={uploading} style={{ ...S.btnGhost, whiteSpace:"nowrap", padding:"8px 14px" }}>
            {uploading ? "…" : "📎 Файл"}
          </button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
        <Field label="Витрати ($)"><input style={num} type="number" value={f.spend}    onChange={set("spend")} /></Field>
        <Field label="Дохід ($)">  <input style={num} type="number" value={f.revenue}  onChange={set("revenue")} /></Field>
        <Field label="CTR (%)">    <input style={num} type="number" value={f.ctr}      onChange={set("ctr")} /></Field>
        <Field label="CR (%)">     <input style={num} type="number" value={f.cr}       onChange={set("cr")} /></Field>
        <Field label="Install">    <input style={num} type="number" value={f.installs} onChange={set("installs")} /></Field>
        <Field label="Regy">       <input style={num} type="number" value={f.regy}     onChange={set("regy")} /></Field>
        <Field label="FTD">        <input style={num} type="number" value={f.ftd}      onChange={set("ftd")} /></Field>
      </div>

      <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:16 }}>
        <button onClick={onClose} style={S.btnGhost}>Скасувати</button>
        <button onClick={() => onSave(f)} disabled={loading||uploading} style={{ ...S.btn, opacity:(loading||uploading)?0.7:1 }}>{loading?"Збереження…":"Зберегти"}</button>
      </div>
    </div>
  );
};
...
// ─── CREATIVES TAB ────────────────────────────────────────────────────────
const CreativesTab = ({ user, domains }) => {
  const [rows, setRows] = useState([]);
  const [modal, setModal] = useState(null);
  const [filter, setFilter] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const handleSave = async (f) => {
    ...
    if (modal.mode === "add") {
      ...
      if (error) showToast("Помилка збереження", "error");
      else showToast("Крео додано ✓");
    } else {
      const { error } = await supabase.from("creatives").update(payload).eq("id", f.id);
      if (error) showToast("Помилка оновлення", "error");
      else showToast("Збережено ✓");
    }
    setSaving(false);
  ...
  const del = async (id) => {
    if (!confirm("Видалити крео?")) return;
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ textAlign:"center", color:"#475569", padding:40 }}>Завантаження…</div> : (
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#13151c", borderRadius:10, overflow:"hidden" }}>
            <thead>
              <tr>
                <Th>Прев'ю</Th><Th>Назва</Th><Th>Домен/ПВА</Th><Th>Статус</Th><Th>Байєр</Th>
                <Th>Витрати</Th><Th>Дохід</Th><Th>ROI</Th><Th>CTR</Th><Th>CR</Th>
                <Th>Install</Th><Th>Regy</Th><Th>FTD</Th><Th>Дата</Th><Th></Th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={15} style={{ padding:40, textAlign:"center", color:"#475569" }}>Немає креативів — додайте перший</td></tr>}
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id} onMouseEnter={e=>e.currentTarget.style.background="#1e2330"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                    <Td>
                      {c.preview_url
                        ? <img src={c.preview_url} alt="" onClick={() => setZoom(c.preview_url)} style={{ width:48, height:48, objectFit:"cover", borderRadius:6, cursor:"zoom-in", border:"1px solid #2e3240" }} onError={e=>e.target.style.display="none"} />
                        : <div style={{ width:48, height:48, background:"#1e2330", borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", color:"#475569", fontSize:20 }}>🖼</div>
                      }
                    </Td>
                    <Td style={{ fontWeight:600, color:"#e2e8f0" }}>{c.name}</Td>
                    <Td>
                      {dm ? <div><div style={{ color:"#60a5fa", fontSize:12 }}>{dm.domain}</div><div style={{ color:"#64748b", fontSize:11 }}>{dm.pwa}</div></div>
                    ...
                    <Td>
                      <button onClick={() => setModal({ mode:"edit", data:c })} style={{ background:"none", border:"none", color:"#60a5fa", cursor:"pointer", fontSize:14 }}>✏️</button>
                      <button onClick={() => del(c.id)} style={{ background:"none", border:"none", color:"#f87171", cursor:"pointer", fontSize:14 }}>🗑</button>
                    </Td>
                  </tr>
                );
              })}
      ...
      {zoom && (
        <div onClick={() => setZoom(null)} style={{ position:"fixed", inset:0, background:"#000c", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", cursor:"zoom-out" }}>
          <img src={zoom} alt="" style={{ maxWidth:"90vw", maxHeight:"90vh", borderRadius:10 }} />
        </div>
      )}
      {modal && (
        <Modal title={modal.mode==="add"?"Новий креатив":"Редагувати креатив"} onClose={() => setModal(null)}>
          <CreativeForm initial={modal.data} domains={domains} onSave={handleSave} onClose={() => setModal(null)} loading={saving} userId={user.id} />
        </Modal>
      )}
    </div>
  );
...
const StatsTab = ({ domains }) => {
  ...
  const totals = useMemo(() => filtered.reduce((acc,c) => ({
    ...
    installs: acc.installs + (parseInt(c.installs)||0),
    regy:     acc.regy     + (parseInt(c.regy)||0),
    ftd:      acc.ftd      + (parseInt(c.ftd)||0),
  ...
  const CARDS = [
    { label:"Витрати",  val:`$${totals.spend.toFixed(0)}`,   color:"#f87171" },
    { label:"Дохід",    val:`$${totals.revenue.toFixed(0)}`, color:"#4ade80" },
    { label:"ROI",      val:totalRoi ? `${totalRoi}%`:"—",   color: !totalRoi ? "#64748b" : totalRoi>=0 ? "#4ade80":"#f87171" },
    { label:"Install",  val:totals.installs, color:"#a78bfa" },
    { label:"Regy",     val:totals.regy,     color:"#a78bfa" },
    { label:"FTD",      val:totals.ftd,      color:"#fbbf24" },
    { label:"Крео",     val:filtered.length, color:"#60a5fa" },
  ];
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:12, marginBottom:20 }}>
        {CARDS.map(c => (
          <div key={c.label} style={S.card}>
            <div style={{ color:"#64748b", fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em" }}>{c.label}</div>
            <div style={{ color:c.color, fontSize:22, fontWeight:800, marginTop:4 }}>{c.val}</div>
          </div>
        ))}
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:10, marginBottom:16 }}>
        <select style={{ ...S.inp, cursor:"pointer" }} value={filters.buyer} onChange={setF("buyer")}>
          <option value="">Всі байєри</option>
          {buyers.map(b=><option key={b}>{b}</option>)}
        </select>
        <select style={{ ...S.inp, cursor:"pointer" }} value={filters.domain_id} onChange={setF("domain_id")}>
          <option value="">Всі домени</option>
          {domains.map(d=><option key={d.id} value={d.id}>{d.domain}</option>)}
        </select>
        <select style={{ ...S.inp, cursor:"pointer" }} value={filters.status} onChange={setF("status")}>
      ...
      {loading ? <div style={{ textAlign:"center", color:"#475569", padding:40 }}>Завантаження…</div> : (
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", background:"#13151c", borderRadius:10, overflow:"hidden" }}>
            <thead>
              <tr>
                <Th>Назва</Th><Th>Домен</Th><Th>Статус</Th><Th>Байєр</Th>
                <Th onClick={()=>tg("spend")}    dir={sd("spend")}>Витрати</Th>
                <Th onClick={()=>tg("revenue")}  dir={sd("revenue")}>Дохід</Th>
                <Th onClick={()=>tg("roi")}      dir={sd("roi")}>ROI</Th>
                <Th onClick={()=>tg("ctr")}      dir={sd("ctr")}>CTR</Th>
                <Th onClick={()=>tg("cr")}       dir={sd("cr")}>CR</Th>
                <Th onClick={()=>tg("installs")} dir={sd("installs")}>Install</Th>
                <Th onClick={()=>tg("regy")}     dir={sd("regy")}>Regy</Th>
                <Th onClick={()=>tg("ftd")}      dir={sd("ftd")}>FTD</Th>
              </tr>
            </thead>
            <tbody>
              {sorted.length === 0 && <tr><td colSpan={12} style={{ padding:40, textAlign:"center", color:"#475569" }}>Немає даних за фільтрами</td></tr>}
              {sorted.map(c => {
                const roi = c.spend > 0 ? (((c.revenue-c.spend)/c.spend)*100).toFixed(1) : null;
                const roiColor = roi===null?"#64748b":roi>=0?"#4ade80":"#f87171";
                const dm = domains.find(d=>d.id===c.domain_id);
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0b0d14" }}>
      <div style={{ borderBottom:"1px solid #1e2330", background:"#0f1117", padding:"0 24px", display:"flex", alignItems:"center" }}>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ background:"none", border:"none", color:tab===t.id?"#e2e8f0":"#64748b", padding:"18px 16px", cursor:"pointer", fontSize:14, fontWeight:tab===t.id?700:400, borderBottom:tab===t.id?"2px solid #3b82f6":"2px solid transparent", marginBottom:-1 }}>{t.label}</button>
        ))}
        <div style={{ flex:1 }} />
        <span style={{ color:"#475569", fontSize:13, marginRight:16 }}>👤 {userName}</span>
        <button onClick={() => supabase.auth.signOut()} style={{ ...S.btnGhost, padding:"6px 14px", fontSize:13 }}>Вийти</button>
      </div>

      <div style={{ padding:24, maxWidth:1600, margin:"0 auto" }}>
        {tab==="domains"   && <DomainsTab   user={user} />}
        {tab==="creatives" && <CreativesTab user={user} domains={domains} />}
